/* NovaBot Desktop v7.5 – Widget + Mobile Redirect
 * Repo: novabot-desktop
 * Author: Mohammed Abu Snaina – NOVALINK.AI
 */

(function () {
  const DESKTOP_CSS_URL =
    "https://novalink2020-hub.github.io/novabot-desktop/novabot-desktop.css";

  const MOBILE_APP_URL =
    "https://novalink2020-hub.github.io/novabot-chat-ui/";
  ...
  function injectDesktopCss() {
    if ([...document.styleSheets].some((s) => s.href && s.href.includes("novabot-desktop.css"))) {
      return;
    }
    const link = document.createElement("link");
    link.rel = "stylesheet";
    link.href = DESKTOP_CSS_URL;
  ...
  /* ------------ Mobile / Tablet: Floating Button → New Tab ------------ */
  function initMobileFabOnly() {
    injectDesktopCss();
    ...
    // نبقي حركة الاهتزاز من حين لآخر
    setInterval(() => {
      if (!fabBtn.classList.contains("nova-idle")) {
  ...
  /* ------------ Desktop Widget: Full 6.9 Experience ------------ */
    ...
    function createBotLeadCard() {
      ...
      btn.addEventListener("click", () => {
        ...
        window.location.href =
          `mailto:${NOVA_CLIENT_CONFIG.CONTACT_EMAIL}` +
